...
public void cargarFichas(List<FichaMedica> lista1, JTable tabla) {
    ...
    String columna[] = {"#", "Cedula", "Nombres", "Apellidos", "Area"};
    ...
    for (int i = 0; i < lista1.size(); i++) {
        filas[0] = "" + (i + 1);
        filas[1] = lista1.get(i).getId_persona();
        var obj = crudP.getOne(lista1.get(i).getId_persona());
        filas[2] = obj.getNombre();
        filas[3] = obj.getApellidos();
        var area = crudA.getOne(obj.getId_area());
        filas[4] = area.getNombre_area();
        modelo.addRow(filas);                        
    }
